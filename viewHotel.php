...
<div class="middle">
  <h1>Hotels</h1>
    <?php
      ...
      $stmt=queryFetchHotel($db,$records_per_page,$offset);
      $result = $stmt->execute();
    ...
    while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
      ...
      echo "
            <tbody>
              ...
              <td>$postcode</td>
              <td>$phone</td>
              <td><form method='POST' action='updatehotel.php'>
        <input type='hidden' name='hotel_ID' value='" . $row['hotel_ID'] . "'>
        <button type='submit' name='askdelete'>Update</button></form></td>
              
              <td>
    <form method='POST' action='confirmdeletehotel.php'>
        <input type='hidden' name='hotel_ID' value='" . $row['hotel_ID'] . "'>
        <button type='submit' name='askdelete'>Delete</button></form>
        </td>
